...
{
    public class GraphMap<T> : Control
    {
        public GraphMap()
        {
			InitializeComponent();
		...
		public class Box : ILinkable {
			...
			public Set<ILinkable> Edges {
				get {
					return links;
				}
			}

			internal Set<ILinkable> links = new Set<ILinkable>();
		...
		public void Save(string filename) {
            ...
            {
                edges = ArrayUtils.ConvertAll(ArrayUtils.ToArray(x.Edges), y => linkable[y]),
                x = x.TopLeft.X,
                y = x.TopLeft.Y,
